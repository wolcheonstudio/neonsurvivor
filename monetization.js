...
const NeonAds = {
    ...
    showRewardedAd(onRewardGranted, onAdSkippedOrError) {
        ...
        if (typeof window.adBreak !== "function") {
            ...
            this.showSimulatedAd(onRewardGranted);
            return;
        }
        ...
        window.adBreak({
            ...
            adBreakDone: (placementInfo) => {
                ...
                if (placementInfo.breakStatus === "notReady" || placementInfo.breakStatus === "timeout") {
                    console.warn("[Google Ads] adBreak notReady. Using beautiful simulated ad screen fallback.");
                    this.showSimulatedAd(onRewardGranted);
                }
            }
        });
    ...
    showSimulatedAd(onRewardGranted) {
        if (window.bgMusic) window.bgMusic.pause();
        ...
        adTitle.textContent = isKo ? '📺 보상형 스폰서 광고 시청' : '📺 Sponsored Sponsor Ad';
        ...
        adSub.textContent = isKo 
          ? '광고 시청 후 게임이 진행됩니다. 조금만 기다려주세요!' 
          : 'Game will resume immediately after this short sponsor ad!';
